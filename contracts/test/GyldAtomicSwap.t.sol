...
contract GyldAtomicSwapTest is Test {
    ...
    //
    // What the ZERO-slot state is NOT: the live deployment. The Sepolia proxy
    // 0x7036206Fc1eBDF8917836b67375E6D49Bc02aBE8 came from commit 46050ea, which already
    // had `maxQuoteTtl` in the struct at B+8 and SEEDED it in `initialize` (that
    // revision's DEFAULT_MAX_QUOTE_TTL was 1 hour). Its slot reads 3600, so it upgrades
    // straight past the fallback into a 1-hour cap — legal, exactly
    // MAX_QUOTE_TTL_CEILING, but 4x the 15-minute default a fresh deploy gets, and only
    // an explicit timelocked setMaxQuoteTtl(15 minutes) narrows it. That proxy predates
    // the BOUNDS, not the slot. test_upgrade_preservesAdminSetTtl below is the shape of
    // that case: a non-zero slot survives upgrade untouched by the fallback.
