...
|---|---|---|
| **Taker binding** (`m.taker == msg.sender`) | swap | Quote theft / MEV-bot execution of someone else's quote. Quotes are not bearer paper (0x mandatory-taker pattern). Note: Backed's production swap does *not* enforce this — we deliberately do. **Re-verified on mainnet 2026-07-31 and this claim STANDS.** Their Team Omega audit (A1, medium) recommended exactly `msg.sender == incomingTransfer.from` and the report marks it "[resolved]" — but the live proxy `0x837E5a6E45F5F16C7306B591994DBD2AdF09A932` does not implement it. `executeSwap` checks expiry, quote reuse, signature and an address allowlist; it never reads `msg.sender`. Proved empirically by replaying a real signed swap from `0x…dEaD` (not a counterparty, not allowlisted) at the prior block — it succeeded. Their line 204 doc comment claims the binding exists; the code does not. **Do not trust an audit's "[resolved]" label as evidence about a deployment** |
...
|---|---|
| `executeSwap(SwapMessage, signature, Permit)` shape: signed quote + single-use quoteId + two-`transferFrom`-leg + optional EIP-2612 permit | Backed Finance `AtomicSwapUpgradeable` — verified on-chain source: [impl #1](https://eth.blockscout.com/address/0x202BDae6EA5CB576c916cF2D2A83d5a21ea2624D?tab=contract), [impl #2](https://eth.blockscout.com/address/0x3AdF98F5eF70E08af964f33D109Ac032b3d31b24?tab=contract); [audit PDF](https://github.com/backed-fi/audits/blob/main/atomic-swap-audit-report-10-2025.pdf). The direct structural twin. **Verified against the live mainnet deployment 2026-07-31** — proxy `0x837E5a6E45F5F16C7306B591994DBD2AdF09A932` (impl `0x202BDae6…`, prior `0x3AdF98F5…`; both are implementations, not the proxy). `struct Transfer { address from; address to; address token; uint256 amount; }` and `SwapMessage { bytes32 quoteId; uint256 expiration; Transfer incomingTransfer; Transfer outgoingTransfer; }` — confirmed by matching the on-chain `TRANSFER_TYPEHASH` `0x41825e53…f3a4` against `keccak("Transfer(address from,address to,address token,uint256 amount)")`. Their contract is a **pure conduit holding no balance**: the only transfer site is `token.safeTransferFrom(transfer.from, transfer.to, transfer.amount)`, `address(this)` appears solely as the permit spender, and the proxy holds 0 USDC / 0 ETH. Both legs settle against `0x5F7A4c11…` (the "Backed: Deployer" EOA, which is also proxy `owner()`). Allowances are **asymmetric**: issuer→proxy is bounded (~194,120 USDC), taker→proxy is `uint256.max`. We retain taker binding, epoch mass-cancel and the NAV band as real differentiators. We no longer have a vault (removed in GYL-548) — we hold inventory in the swap itself, which is neither their design nor our original one |
