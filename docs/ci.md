...
|-----|--------------|---------------------------|
| `test` | `forge build` + `forge test` at **full** `foundry.toml` intensity (fuzz `runs = 10000`, invariant `runs = 1000, depth = 50`, 487 tests) | Regressions in contracts, scripts and invariants landing on `main` unnoticed |
