...
|-----|--------------|---------------------------|
| `test` | `forge build` + `forge test` at **full** `foundry.toml` intensity (fuzz `runs = 10000`, invariant `runs = 1000, depth = 50`; 524 tests across 20 suites as of 2026-08-26, confirmed with `forge test --list`) | Regressions in contracts, scripts and invariants landing on `main` unnoticed |
| `slither` | `pip install slither-analyzer==0.11.6` + `python3 ci/check_slither.py`. Runs Slither **unfiltered**, filters to `contracts/*.sol` (excluding `test/` and `script/`) on the repo-relative path, and fails if the set of `(detector, file, element)` fingerprints has **grown** beyond `ci/slither-baseline.json`. Every baselined finding is triaged in [`docs/audit/known-issues.md`](audit/known-issues.md) | New static-analysis findings landing unnoticed. Gated on a baseline rather than severity because the ~48 production findings span Informational to Medium and **none is a live defect** — a severity gate would either pass everything or block on five `incorrect-equality` hits against a `_updatedAt == 0` sentinel, i.e. the most clearly false results in the set. Keyed on element *name*, not line number, so unrelated edits above a finding do not churn the baseline. **Do not add `--filter-paths "lib/"`**: Slither matches it against the absolute path, so in a checkout whose own path contains `lib/` — the vendored `kaleidoscope/lib/gyld-contracts` — it filters out the whole project and prints a confident `0 result(s) found`. Removing code that had a baselined finding prints a NOTE and does not fail |
| `halmos` | `pip install halmos==0.3.3` + `python3 ci/check_halmos.py`. Runs the four `check_` properties in `GyldAtomicSwap.halmos.t.sol` symbolically and holds each to the outcome recorded in [`docs/audit/formal-verification.md`](audit/formal-verification.md): three PASS, one TIMEOUT | A proved invariant silently becoming unprovable, and — the loud case — the recorded timeout turning into a **counterexample**, which would mean I-11 is genuinely violated. Gated per-property rather than on Halmos's exit code, because Halmos exits non-zero on any non-proof *including timeouts*, so a bare invocation is permanently red and would be ignored within a week. A TIMEOUT that starts passing also fails, with an instruction to promote it — an unproved property quietly becoming proved is news the record should carry. **Do not set `--solver-timeout-assertion 0`**: zero means *no* timeout and the redeem property then hangs indefinitely, which reads as a slow job rather than a stuck one. `forge test` never runs these — the `check_` prefix is invisible to Foundry's `test_` convention |
...
`slither` was added under exactly that bar: it started green on a 44-entry
triaged baseline, it fails only when the finding set grows, and its failure
message names each new finding and points at the triage document rather than at
