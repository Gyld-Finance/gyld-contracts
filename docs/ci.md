...
|-----|--------------|---------------------------|
| `test` | `forge build` + `forge test` at **full** `foundry.toml` intensity (fuzz `runs = 10000`, invariant `runs = 1000, depth = 50`; 535 tests across 20 suites as of 2026-08-20, confirmed with `forge test --list`) | Regressions in contracts, scripts and invariants landing on `main` unnoticed |
...
- **`forge build --deny-warnings`** — two live solc warnings (state mutability,
  ...
  to `pure` at `contracts/test/GyldAtomicSwap.spec.t.sol:608`) plus ~40
  `forge lint` notes
