...
| `0x573fc08b` | `QuoteExpiryTooFar(uint64 expiry, uint64 maxAllowed)` | The quote's `expiry` is further than `maxQuoteTtl()` (default 1 hour) in the future. The bound is inclusive: `expiry == block.timestamp + maxQuoteTtl` passes. | Request a shorter-dated quote; do not sign or hold long-dated quotes. `maxAllowed` returns the current ceiling. |
...
| `0x5344476d` | `StaleNav(address token, uint256 updatedAt)` | The NAV feed is older than `maxNavAgeSecs()`, or reports a **future-dated** `updatedAt`. **Fails closed by design.** | **Transient infrastructure condition** — retry with backoff. Not a bad-input error. |
...
| `0x3a2d1e17` | `InvalidTokenDecimals(address token, uint8 decimals)` | Admin path — `registerSeries` probe rejected a bond token not reporting **18** decimals, or `initialize` was given a cash token not reporting **6** decimals. `decimals == 0` means the address has no usable `decimals()` at all. | Admin-only. The 18dp/8dp/6dp ladder is what the NAV band's `/1e20` scaling assumes — never bypass. |
...
| `0x29aaa1be` | `CannotRenouncePauserRole()` | `renounceRole(PAUSER_ROLE, …)` is blocked — a sole pauser self-renouncing would remove the fast incident-response halt until the timelock re-grants. | Admin-only; use `revokeRole` as an explicit two-party action. |
| `0x6b0b4098` | `CannotRenounceTreasurerRole()` | `renounceRole(TREASURER_ROLE, …)` is blocked — a sole treasurer self-renouncing would remove the paused-state evacuation path (`withdraw`) until the timelock re-grants. | Admin-only; use `revokeRole` as an explicit two-party action. |
