...
Verified against the Solidity on `main` @ `08f575f` (535 tests,
...
guard do not have it and still depend on the runbook rule. The feed is not
...
- **The live Sepolia integrator instance predates GYL-1134/1135.** It was deployed
  from the pre-hardening scripts, so it lacks the NAV-age ceiling
  ([§5.7](#the-nav-staleness-ceiling--the-single-most-load-bearing-constant-here))
  and the fail-closed deploy guards ([§13.3](#133-the-guard-surface)). Its wiring is
  dev-mode throughout: the deployer EOA holds every role, with no timelock handover,
  and the one timelock in the register has `minDelay = 0` and an open executor. Treat
  it as a demonstration, not as a template — redeploy or upgrade before anything
  depends on it.
...
state-mutability warnings in test code, ~40 `forge lint` notes, and a tree that is
